...
public final class TrainListTask extends BaseTask {
    private String mID;
    private String mPageNo;
    private String mPageSize;
    private String mUrl;

    public TrainListTask(Context ctx, String remote_address, Handler main_handler,
                         String id,String pageNo,String pageSize, String user_token,
                         boolean is_granted) {
        super(ctx, remote_address, user_token, main_handler, is_granted);
        mID = id;
        mPageNo=pageNo;
        mPageSize=pageSize;
        mUrl = mRemoteServerAddress + UserSystemConfig.URI_TRAIN_LIST;
    }

    @Override
    public String doTask() {
        String main_json_str = UserSystemUtils.createTrainListInfoMainRequest(mID,mPageNo,mPageSize);
    ...
    public void onSuccess(String result_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_TRAIN_LIST_MESSAGE_SUCCESS);
    ...
    public void onFalse(String false_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_TRAIN_LIST_MESSAGE_FALSE);
    ...
    public void onException() {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_TRAIN_LIST_MESSAGE_EXCEPTION);
            success_message.sendToTarget();
        }
    }
}
