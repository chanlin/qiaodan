...
public final class MoveUploadsTask extends BaseTask {
    private ArrayList<MoveUploadInfo> mlist;
    private String mUrl;

    public MoveUploadsTask(Context ctx, String remote_address, Handler main_handler,
                           ArrayList<MoveUploadInfo> list,String user_token,
                           boolean is_granted){
        super(ctx, remote_address, user_token, main_handler, is_granted);
        mlist=list;
        mUrl = mRemoteServerAddress + UserSystemConfig.URI_MOVE_UPLOADS;
    }

    @Override
    public String doTask() {
        String main_json_str = UserSystemUtils.createMoveUploadsMainRequest(mlist);
        Log.i("doTask","doTask main_json_str:"+main_json_str);
        ...
        Log.i("doTask","doTask result_json:"+result_json);
        return result_json;
    }
    ...
    public void onSuccess(String result_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_MOVE_UPLOADS_MESSAGE_SUCCESS);
    ...
    public void onFalse(String false_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_MOVE_UPLOADS_MESSAGE_FALSE);
    ...
    public void onException() {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_MOVE_UPLOADS_MESSAGE_EXCEPTION);
            success_message.sendToTarget();
        }
    }
}
