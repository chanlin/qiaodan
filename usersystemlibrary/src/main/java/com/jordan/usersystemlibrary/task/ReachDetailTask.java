...
public final class ReachDetailTask extends BaseTask {
    private String mID;
    private String mUrl;

    public ReachDetailTask(Context ctx, String remote_address, Handler main_handler,
                           String id, String user_token,
                           boolean is_granted) {
        super(ctx, remote_address, user_token, main_handler, is_granted);
        mID = id;
        mUrl = mRemoteServerAddress + UserSystemConfig.URI_REACH_DETAIL;
    }

    @Override
    public String doTask() {
        String main_json_str = UserSystemUtils.createReachDetailMainRequest(mID);
    ...
    public void onSuccess(String result_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_REACH_DETAIL_MESSAGE_SUCCESS);
    ...
    public void onFalse(String false_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_REACH_DETAIL_MESSAGE_FALSE);
    ...
    public void onException() {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_REACH_DETAIL_MESSAGE_EXCEPTION);
            success_message.sendToTarget();
        }
    }
}
