...
public final class ShoesRecoTask extends BaseTask {
    private String mUrl;


    public ShoesRecoTask(Context ctx, String remote_address, Handler main_handler,
                         String user_token,
                         boolean is_granted) {
        super(ctx, remote_address, user_token, main_handler, is_granted);
        mUrl = mRemoteServerAddress + UserSystemConfig.URI_SHOES_RECO;
    }

    @Override
    public String doTask() {
        String main_json_str = UserSystemUtils.createShoesRecoMainRequest();
    ...
    public void onSuccess(String result_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_SHOES_RECO_MESSAGE_SUCCESS);
    ...
    public void onFalse(String false_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_SHOES_RECO_MESSAGE_FALSE);
    ...
    public void onException() {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_SHOES_RECO_MESSAGE_EXCEPTION);
            success_message.sendToTarget();
        }
    }
}
