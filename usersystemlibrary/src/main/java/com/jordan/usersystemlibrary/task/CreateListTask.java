...
public final class CreateListTask extends BaseTask {
    private String mPageNo;
    private String mPageSize;
    private String mUrl;

    public CreateListTask(Context ctx, String remote_address, Handler main_handler,
                          String pageNo, String pageSize, String user_token,
                          boolean is_granted) {
        super(ctx, remote_address, user_token, main_handler, is_granted);
        mPageNo=pageNo;
        mPageSize=pageSize;
        mUrl = mRemoteServerAddress + UserSystemConfig.URI_CREATE_LIST;
    }

    @Override
    public String doTask() {
        String main_json_str = UserSystemUtils.createCreateListMainRequest(mPageNo,mPageSize);
    ...
    public void onSuccess(String result_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_CREATE_LIST_MESSAGE_SUCCESS);
    ...
    public void onFalse(String false_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_CREATE_LIST_MESSAGE_FALSE);
    ...
    public void onException() {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_CREATE_LIST_MESSAGE_EXCEPTION);
            success_message.sendToTarget();
        }
    }
}
