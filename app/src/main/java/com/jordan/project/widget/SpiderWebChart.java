...
public class SpiderWebChart extends View {
	...
	protected void drawSpiderWeb(Canvas canvas){
		...
		mPaintWebFill.setColor(0x00000000);
		mPaintWebFill.setAntiAlias(true);
		
		Paint mPaintWebBorder =new Paint();
		mPaintWebBorder.setColor(0xFF848484);
		//边框
		...
		mPaintWebInnerBorder.setColor(0x00000000);
		...
		mPaintFont.setColor(0xFF848484);
		mPaintFont.setTextSize(WindosUtils.windowsWidth/20);//设置字体大小
	...
	protected void drawData(Canvas canvas) {
		LogUtils.showLog("redar", "drawData");
		if (null != data) {
			// 遍历每条数据列表
			for (int j = 0; j < data.size(); j++) {
				...
				for (int i = 0; i < pointList.size(); i++) {
					...
					canvas.drawCircle(pt.x, pt.y, WindosUtils.windowsWidth/100, mPaintPoint);
				}
				mPath.close();
	...
	public void setData(List<List<TitleValueEntity>> data) {
		LogUtils.showLog("redar", "setData");
		this.data = data;
	}
