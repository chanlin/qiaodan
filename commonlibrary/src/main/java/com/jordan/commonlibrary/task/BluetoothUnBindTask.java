...
public final class BluetoothUnBindTask extends BaseTask {
    private String mIDS;
    private String mUrl;

    public BluetoothUnBindTask(Context ctx, String remote_address, Handler main_handler,
                               String ids, String user_token,
                               boolean is_granted) {
        super(ctx, remote_address, user_token, main_handler, is_granted);
        mIDS = ids;
        mUrl = mRemoteServerAddress + CommonSystemConfig.URI_BLUETOOTH_UNBIND;
    }
    @Override
    public String doTask() {
        String main_json_str = CommonSystemUtils.createBluetoothUnBindMainRequest(mIDS);
    ...
    public void onSuccess(String result_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.COMMON_BLUETOOTH_UNBIND_MESSAGE_SUCCESS);
    ...
    public void onFalse(String false_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.COMMON_BLUETOOTH_UNBIND_MESSAGE_FALSE);
    ...
    public void onException() {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.COMMON_BLUETOOTH_UNBIND_MESSAGE_EXCEPTION);
            success_message.sendToTarget();
        }
    }
}
